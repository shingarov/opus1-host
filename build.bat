cl LED.c LED_main.c CRASH.c MIDI.c serial_io.c winmm.lib
cl drawknob.c CRASH.c MIDI.c serial_io.c winmm.lib
rem cl Debug_LED.c CRASH.c serial_io.c LED.c
rem cl Debug_LED_2.c CRASH.c serial_io.c LED.c
cl console.c console_led.c console_piston.c CRASH.c serial_io.c MIDI.c winmm.lib
rem cl Debug_console_LED.c CRASH.c serial_io.c console.c
rem cl Debug_console_input.c CRASH.c serial_io.c

